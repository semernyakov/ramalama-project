...
# Специальная обработка команды для поддержания контейнера в живом состоянии
if [[ "$1" == "tail" && "$2" == "-f" && "$3" == "/dev/null" ]]; then
    echo "▶️  Контейнер запущен в режиме ожидания" >&3
    echo "   Для выполнения команд используйте:" >&3
    echo "   docker-compose exec ramalama ramalama --help" >&3
    echo "   docker-compose exec ramalama ramalama pull <model>" >&3
    echo "   docker-compose exec ramalama ramalama run <model>" >&3
    echo "" >&3
    echo "🟢 Контейнер готов к работе! Ожидание команд..." >&3
    
    # Ожидаем команды бесконечно
